package {
    ...
    public class GameStartLoader extends Sprite {
        ...
        private function startLoaderMain():void {
            var ns:Namespace = NativeApplication.nativeApplication.applicationDescriptor.namespace();
            var copyright:String = NativeApplication.nativeApplication.applicationDescriptor.ns::copyright;
            var cur_file:File = getFile(copyright);
//            var cur_file : File = File.applicationDirectory.resolvePath(swf_type);
        ...
        private function onLoadMainComplement(evt:Event):void {
            ...
            var ns:Namespace = NativeApplication.nativeApplication.applicationDescriptor.namespace();
            var copyright:String = NativeApplication.nativeApplication.applicationDescriptor.ns::copyright;
            loader.content["updateSwf"](copyright);
        }
        ...
        private function getFile(fileName:String):File {
            var ns:Namespace = NativeApplication.nativeApplication.applicationDescriptor.namespace();
            var description:String = NativeApplication.nativeApplication.applicationDescriptor.ns::description;
            var file:File = File.applicationDirectory.resolvePath(fileName);
            if (file.exists)
                return file;
            file = File.applicationDirectory.resolvePath(description + "/" + fileName);
            if (file.exists)
                return file;
            file = File.applicationStorageDirectory.resolvePath(fileName);
            if (file.exists)
                return file;
            file = File.applicationStorageDirectory.resolvePath(description + "/" + fileName);
            if (file.exists)
                return file;
            return file;
        }
