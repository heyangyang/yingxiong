...
//		}
		...
		private function savePlayState():void
		{
			...
			LocalShareManager.getInstance().save(LocalShareManager.SOUND, str);
		}
		...
		public function initPlayState():void
		{
			...
			var abc:String=LocalShareManager.getInstance().get(LocalShareManager.SOUND);
...
//				setMusicState(_isMusicOn);
		...
		/** Play a sound */
		public function playSound(id:String, stopSame:Boolean=false, volume:Number=0.75, repetitions:int=1, panning:Number=0):void
		{
...
//			{
            ...
            var soundObject:Sound= AssetMgr.instance.getSound(id);
            if(!soundObject)
            {
                soundObject = BattleAssets.instance.getSound(id);;
            }
				...
				var s:SoundTransform=new SoundTransform(v* 0.5, panning);
