...
package feathers.dragDrop {
    ...
    public class DragDropManager {
         ...
         */
        public static function startDrag(source:IDragSource, touch:Touch, data:DragData, dragAvatar:DisplayObject = null,
                                         dragAvatarOffsetX:Number = 0, dragAvatarOffsetY:Number = 0):void {
            if (isDragging) {
                cancelDrag();
            }
            if (!source) {
        ...
        protected static function updateDropTarget(location:Point):void {
            ...
            if (target != dropTarget) {
                if (dropTarget) {
                    ...
                    dropTarget.dispatchEvent(new DragDropEvent(DragDropEvent.DRAG_EXIT, _dragData, false, dropTargetLocalX,
                                                               dropTargetLocalY));
                }
                ...
                if (dropTarget) {
                    ...
                    dropTarget.dispatchEvent(new DragDropEvent(DragDropEvent.DRAG_ENTER, _dragData, false, dropTargetLocalX,
                                                               dropTargetLocalY));
                }
            } else if (dropTarget) {
                dropTargetLocalX = location.x;
        ...
        protected static function stage_touchHandler(event:TouchEvent):void {
            ...
            if (touch.phase == TouchPhase.MOVED) {
                ...
                if (avatar) {
                    avatar.x = (HELPER_POINT.x / Constants.scale) + avatarOffsetX;
                    avatar.y = (HELPER_POINT.y / Constants.scale) + avatarOffsetY;
                }
                updateDropTarget(HELPER_POINT);
                ...
                if (dropTarget && isAccepted) {
                    dropTarget.dispatchEvent(new DragDropEvent(DragDropEvent.DRAG_DROP, _dragData, true, dropTargetLocalX,
                                                               dropTargetLocalY));
                    isDropped = true;
                }
                dropTarget = null;
