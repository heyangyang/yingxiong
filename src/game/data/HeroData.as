    ...
    public class HeroData extends Data {
        ...
        public function getNextPurgePropertys(nextQuality:int):Array {
            ...
            for (i = 0; i < l; i++) {
                ...
                var nextValue:int = JTFormulaUtil.getUpProperty(JTFormulaUtil.getNextPurgeDeff(bvaseValue, cHeroInfo.arg,
                                                                                               nextHeroData.arg), level);
        ...
        public function getNextStarPropertys(nextStar:int):Array {
            ...
            for (i = 0; i < l; i++) {
                ...
                list[i] = JTFormulaUtil.getUpProperty(nextValue - bvaseValue, level);
            }
            return list;
        }
        ...
        public function subEquipProperty(widgetid:int):void {
            if (widgetid > 0) {
                ...
                for (var k:int = 0; k < le; k++) {
                    var key:String = Val.MAGICBALL[k];
        ...
        public function updataLvPropertys(heroInfo:HeroData):void {
            var i:int = 0;
            var len:int = Val.MAGICBALL.length;
            var key:String;
         ...
         *
         */
        public function getHeroCurrEquipList():Vector.<WidgetData> {
            var weapon_id:int;
            var widget:WidgetData;
            var bestWidget:WidgetData;
            var tmp_list:Vector.<WidgetData> = new Vector.<WidgetData>;
            for (var i:int = 1; i <= 5; i++) {
                weapon_id = this["seat" + i];
                widget = WidgetData.hash.getValue(weapon_id) as WidgetData;
                if (weapon_id == 0 || widget == null)
...
//                    skillData.callbackFrame
        ...
        public function get isSelfBattleHero():Boolean {
            return this.team == BLUE && this.id > 0;
        }
    }
