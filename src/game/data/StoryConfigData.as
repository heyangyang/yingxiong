	...
	public class StoryConfigData extends Data
	{
		...
		public static function getAllHalfPhoto(id : int) : Array
		{
			...
			for (var i : int = 1; i <= 3; i++)
			{
				...
				if (data)
					data.getHalfPhoto(tmp_arr);
			}
			return tmp_arr;
		}

		public function getHalfPhoto(list_photo : Array) : void
		{
			...
			for (var i : int = 0; i < len; i++)
			{
				...
				if (storyData.target == 0)
					storyData.target_photo = target.type;

				if (list_photo.indexOf(roleShow.half_photo) == -1)
					list_photo.push(roleShow.half_photo);
			}
		}
	}
}