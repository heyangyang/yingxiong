    ...
    public class UpdateGameScene extends BaseScene {
        ...
        override protected function show():void {
            if (!AirNetworkInfo.networkInfo.isConnectedWithWIFI())
                AccountManager.instance.showAlert(Val.getlange("LOAD_NO_WIFT"), startUpdate, Val.getlange("GOTO"), AccountManager.instance.exitApp,
                                                  Val.getlange("exit"));
            else
                startUpdate();
        }
        ...
        protected function onUpdateHandler(event:UpdateExtensionEvent):void {
            ...
            if (event.updateFlag || !AirNetworkInfo.networkInfo.isConnected()) {
                AccountManager.instance.showAlert(Langue.getLangue("Download_New_Update") + "?", okHander, Val.getlange("OK"),
                                                  AccountManager.instance.exitApp, Val.getlange("exit")); //下载失败,是否从新更新
...
//			{
//				AccountManager.instance.showAlert("更新成功!", NativeApplication.nativeApplication.exit, Val.getlange("OK"));
//			}
        }
        /**
         * 加载背景资源
