package game.view.battle {
    ...
    public class BattleUI extends View {
        ...
        private var effectList:Vector.<SpriterClip> = null;
        ...
        override protected function init():void {
            ...
            effectList = new Vector.<SpriterClip>;
...
//                                (bossView.bossHead.getChildByName("bossIcon") as Image).texture=AssetMgr.instance.getTexture(boss.useIcon);
            ...
            if (GameMgr.instance.sBattle == null) {
                video_animation = AnimationCreator.instance.create("effect_video", AssetMgr.instance);
...
//            }
        ...
        private function createBattleView():void {
            ...
            bossPercent = new TextField(230, 36, '', '', 30, 0xFFFFFF, false);
            bossPercent.touchable = false;
            bossPercent.hAlign = 'center';
            bossPercent.text = '';
            bossPercent.x = 15;
            bossPercent.y = 15;
            ...
            tag_speed1.y = Constants.virtualHeight - tag_speed1.height - 20;
            ...
            tag_speed2.y = Constants.virtualHeight - tag_speed2.height - 20;
            ...
            for (i = 0; i < 5; i++) {
                ...
                getEffectSp.stop();
                battleView.addQuiackChild(getEffectSp);
                getEffectSp.x = img.x + 50;
                getEffectSp.y = img.y + 50;
                effectList.push(getEffectSp);
            ...
            i = 0;
            var vect:Vector.<*> = BattleHeroMgr.instance.hash.values();
            for each (var hero:Hero in vect) {
                if (hero.id != 0 && hero.data.team == HeroData.BLUE) {
                    ...
                    heroQuality.texture = Res.instance.getQualityHeroTexture(hero.data.quality);
                    star = battleView.getChildByName("star_" + i) as StarBarRender;
                    star.updataStar(hero.data.foster, 0.9);
                    star.x = heroQuality.x + ((heroQuality.width - star.width) >> 1);
                    star.y = img.height - star.height;
                    i++;
                }
            }
        ...
        private function onChangeSpeed(evt:Event):void {
            ...
            if (speed == 1.0 && vipData) {
                addTips(getLangue("vip_speed").replace("*", vipData.speed_values / 10).replace("*", vipData.id));
            }
        ...
        private function showSpeedType(speed:Number):void {
            ...
            if (tag_speed2.visible) {
                tag_speed2.upState = Res.instance.getVipSpeedPhoto(GameMgr.instance.vipData.baseVip.speed_values / 10);
            }
        }
...
//            }
        ...
        private function updataSp(heroData:HeroData):void {
            ...
            if (index >= 0) {
                ...
                if (scale >= 1) {
                    ...
                    eff.visible = true;
                    eff.play("effect_yingxiongdazhao");
                    Starling.juggler.add(eff);
                }
                maskBarRect(sp, 110, 24, scale);
            }
        }
        ...
        public function freeSkill(heroData:HeroData):void {
            ...
            if (index >= 0) {
                ...
                maskBarRect(sp, 110, 24, 1, 0.1, function():void {

                    Starling.juggler.delayCall(function():void {
                        maskBarRect(sp, 110, 24, 0, 2, function():void {
                            eff.visible = false; //隐藏效果
                            eff.stop();
                            Starling.juggler.remove(eff);

                            var effectSp:SpriterClip = AnimationCreator.instance.create("effect_yingxiongdazhao2", AssetMgr.instance);
                            effectSp.play("effect_yingxiongdazhao2");
                            effectSp.animation.looping = true;
                            effectSp.name = "effectSp_" + index;
                            effectSp.touchable = false;
                            Starling.juggler.add(effectSp);
                            battleView.addQuiackChild(effectSp);
                            effectSp.x = (battleView["heroIcon_" + index] as Image).x + 50;
                            effectSp.y = (battleView["heroIcon_" + index] as Image).y + 50;
                            effectSp.animationComplete.addOnce(function():void {
                                effectSp && effectSp.removeFromParent(true);
                            });
                        });
        ...
        private function getPhotoIndex(heroData:HeroData):int {
            ...
            if (heroData.team == HeroData.BLUE) {
                //找出对应英雄index
                var vect:Vector.<*> = BattleHeroMgr.instance.hash.values();
                for each (var chero:Hero in vect) {
                    if (chero.data.team == HeroData.BLUE) { //如果是自己的英雄
                        if (chero.data.id != 0) { //不是固有英雄
                            index++;
                            if (heroData.id == chero.data.id) { //如果自己英雄==当前英雄下标找到
                                bool = true;
                                break;
                            }
                        }
