package game.view.magicShop.view
{
	...
	public class MagicObrGetView extends MagicObrGetViewBase
	{
		...
		{
			var index:int=1;
			index=int((e.currentTarget as SarahRenderView).name.split("_")[1]);
			getMagicOrbs(index);
		}
		...
		private function showMagicState():void
		{
			...
			for (var i:int=0; i < length; i++)
			{
				...
				serahItem=this.getChildByName("sarah_" + (i + 1)) as SarahRenderView;
		...
		{
			getMagicOrbs(int(sarah.name.split("_")[1]));
		}
		...
		{
			//宝珠状态
			if (_arg1.getName() == String(SMagicOrbsState.CMD))
			{
			...
			else if (_arg1.getName() == SOversellItem.CMD.toString()) //出售宝珠 
			{
		...
		override public function listNotificationName():Vector.<String>
		{
			...
			vect.push(SMagicOrbsState.CMD, SOversellItem.CMD, SGetMagicOrbs.CMD);
			return vect;
		}
