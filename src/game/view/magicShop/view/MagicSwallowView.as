package game.view.magicShop.view
{
	...
	public class MagicSwallowView extends MagicSwallowViewBase
	{
		...
		override protected function addListenerHandler() : void
		{
			...
			addContextListener(SJewelry.CMD + "", handleNotification);
			addContextListener(SStrengthen.CMD + "", handleNotification);
		...
		private function handleNotification(evt : Event, _arg1 : IData) : void
		{
			...
			if (_arg1.getCmd() == SJewelry.CMD)
			{
				...
				if (0 == jewelry.code)
				{
					...
					effectData = {vector: dataVector, effectPoint: null, effectName: "effect_036", effectSound: "baoxiangkaiqihuode", effectFrame: 299};
		...
		private function getJewelLevData() : Object
		{
			...
			var selectedWidget : WidgetData = WidgetData.hash.getValue(getMagicOrbs.id) as WidgetData; // 获取当前升级的宝珠
			var obj : Object = null;
			var arr : Array = ArrayUtil.change2Array(JewelLevData.JewelLevHash.values());
