...
//                curr_scene=SceneMgr.instance.changeScene(sceneClass);
				...
				if (sceneClass == CityFace)
				{
					curr_scene=SceneMgr.instance.LoadResChange(CityFace, null, CityAssets.instance, [ConverURL.conver("home/")], call);
					if (!curr_scene)
					{
						return;
					}
			...
			function call(scene:IViewGuideView):void
			{
				curr_scene=scene;
				curr_scene && (BaseScene(curr_scene).touchable=false);

				if (dialogClass != null && viewGuideData.view != "MsgDialog")
				{
