...
/**
	...
	public class PhoneDevice implements IPhoneDevice
	{
		 ...
		 *
		 */
		public function pay(productId : String, productName : String, productPrice : Number, productCount : int, pay_orderId : String,diamond:int) : void
		{
			orderId = pay_orderId;
			DataEyeManger.instance.onCharge(pay_orderId, productPrice);
			trace("平台充值:", productId, "  ", productName, "  ", productPrice, "  ", productCount);
		}
		...
		{
			LocalShareManager.getInstance().save(PhoneDevice.pay_device + Config.device, orderId);
			LocalShareManager.getInstance().flush();
		}

		protected function get orderId() : String
		{
			return LocalShareManager.getInstance().get(PhoneDevice.pay_device + Config.device);
		}
		...
		public static function sendDiamondShop(verifyReceipt : String) : void
		{
            ...
            trace("[PhoneDevice]","发送充值信息","platform: ",cmd.platform,"receipt: ",cmd.receipt);
		}
	}
}